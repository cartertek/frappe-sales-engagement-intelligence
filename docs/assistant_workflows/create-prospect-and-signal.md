...
8. For a supported path, create or identify the Prospect using the organization's canonical public name. `prospect_name` must identify the organization or entity, never the product, platform, system, migration, initiative, signal, location, or research path. Put that context in the Signal, metadata, or notes. Search for an existing organization-level Prospect before creating one, then link the draft, add a descriptive Signal name, complete structured analysis, and check all managed disqualifiers and guardrails.
