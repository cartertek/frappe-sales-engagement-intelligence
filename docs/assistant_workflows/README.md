...
Before an assistant creates or strengthens a Moderate or Strong signal, it must open each exact URL that will be stored on an `observed_facts` row as `source_url` and confirm that the page contains the expected entity, expected document or role, and the complete verbatim quotation stored on each corresponding Observed Facts row. Discovery snippets, cached text, ATS APIs, job feeds, reconstructed URLs, generic ATS shells, and HTTP status alone are not sufficient. No verified exact source means no qualifying database write.
