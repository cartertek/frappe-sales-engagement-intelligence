...
def _prospect_arena_filter(filters, alias: str = "p") -> tuple[str, dict]:
    arena = (filters or {}).get("research_arena") or (filters or {}).get("arena")
    if not arena:
        return "", {}
    return (
...
def _execute_active_prospect_queue(filters):
    ...
    columns = [_link("Prospect", "prospect", "SEI Prospect"), _data("Prospect Type", "prospect_type"), _data("Research Arenas", "research_arena", 220), _data("Playbooks", "playbooks", 220), _data("Qualification Status", "qualification_status"), _data("Lifecycle Status", "lifecycle_status"), _data("Next Action", "next_action"), _date("Next Action Date", "next_action_date"), _link("Assigned To", "assigned_to", "User"), _data("CRM Leads", "crm_leads", 240), _link("CRM Deal", "crm_deal", "CRM Deal"), _datetime("Modified", "modified")]
    data = _sql(f"""
        SELECT p.name prospect, p.prospect_type, {_prospect_arenas_expr("p")} research_arena, {_prospect_playbooks_expr("p")} playbooks, p.qualification_status, p.lifecycle_status, p.next_action, p.next_action_date, p.assigned_to, {_prospect_crm_leads_expr("p")} crm_leads, p.crm_deal, p.modified
...
def _execute_terminal_status_review(filters):
    ...
    filter_sql = where.replace(" WHERE ", " AND ", 1).replace(utils.table("SEI Prospect"), "p")
    columns = [_link("Prospect", "prospect", "SEI Prospect"), _data("Lifecycle Status", "lifecycle_status"), _data("Qualification Status", "qualification_status"), _data("Rejected Reason", "rejected_reason", 240), _check("Do Not Contact", "do_not_contact"), _data("Research Arenas", "research_arena", 220), _data("Playbooks", "playbooks", 220), _link("Modified By", "modified_by", "User"), _datetime("Modified", "modified")]
    data = _sql(f"""
        SELECT p.name prospect, p.lifecycle_status, p.qualification_status, p.rejected_reason, p.do_not_contact, {_prospect_arenas_expr("p")} research_arena, {_prospect_playbooks_expr("p")} playbooks, p.modified_by, p.modified
        FROM {utils.table('SEI Prospect')} p
...
def _execute_qualification_by_signal_type(filters):
    ...
    data = _sql(f"""
        SELECT COALESCE(st.signal_type_name, s.signal_type) signal_type,
...
def _execute_missing_evidence_report(filters):
    ...
    columns = [_data("Issue Type", "issue_type"), _link("Prospect", "prospect", "SEI Prospect"), _link("Signal", "signal", "SEI Signal"), _data("Research Arenas", "research_arena", 220), _data("Signal Type", "signal_type"), _data("Details", "details", 360), _datetime("Modified", "modified")]
    data = _sql(f"""
        SELECT 'Prospect has no linked SEI Signal' issue_type, p.name prospect, NULL `signal`, {_prospect_arenas_expr('p')} research_arena, NULL signal_type, 'No SEI Signal rows reference this prospect.' details, p.modified
        FROM {utils.table('SEI Prospect')} p LEFT JOIN {utils.table('SEI Signal')} s ON s.prospect = p.name
        WHERE s.name IS NULL
        UNION ALL
        SELECT 'Signal missing source URL', s.prospect, s.name `signal`, NULL research_arena, s.signal_type, 'No fact has a source_url.' details, s.modified FROM {utils.table('SEI Signal')} s WHERE NOT EXISTS (SELECT 1 FROM {utils.table('SEI Signal Observed Fact')} f WHERE f.parent=s.name AND COALESCE(f.source_url,'')!='')
        UNION ALL
        SELECT 'Signal missing source date', s.prospect, s.name `signal`, NULL research_arena, s.signal_type, 'No fact has a source_date.' details, s.modified FROM {utils.table('SEI Signal')} s WHERE NOT EXISTS (SELECT 1 FROM {utils.table('SEI Signal Observed Fact')} f WHERE f.parent=s.name AND f.source_date IS NOT NULL)
        UNION ALL
        SELECT 'Signal missing evidence notes', prospect, name `signal`, NULL research_arena, signal_type, 'evidence_notes is blank.' details, modified FROM {utils.table('SEI Signal')} WHERE COALESCE(evidence_notes, '') = ''
        UNION ALL
        SELECT 'Prospect has no signal-derived arena', p.name prospect, NULL `signal`, NULL research_arena, NULL signal_type, 'No linked Signal Type provides a Research Arena.' details, p.modified FROM {utils.table('SEI Prospect')} p WHERE NOT EXISTS (SELECT 1 FROM {utils.table('SEI Signal')} s INNER JOIN {utils.table('SEI Signal Type')} st ON st.name=s.signal_type WHERE s.prospect=p.name AND COALESCE(st.research_arena,'')!='')
        ORDER BY modified DESC
    """)
    return columns, data
...
def _execute_prospects_by_research_arena(filters):
    ...
    columns = [_link("Research Arena", "research_arena", "SEI Research Arena"), _int("Total Prospects", "total_prospects"), _int("Qualified Prospects", "qualified_prospects"), _int("Manually Approved Prospects", "manually_approved_prospects"), _int("Ready for CRM Conversion", "ready_for_crm_conversion"), _int("Converted to CRM Lead", "converted_to_crm_lead"), _int("Converted to CRM Deal", "converted_to_crm_deal"), _int("Rejected", "rejected"), _int("Do Not Contact", "do_not_contact"), _percent("Qualification Rate", "qualification_rate"), _percent("CRM Lead Conversion Rate", "crm_lead_conversion_rate"), _percent("CRM Deal Conversion Rate", "crm_deal_conversion_rate")]
    data = _sql(f"""
        SELECT st.research_arena research_arena, COUNT(DISTINCT p.name) total_prospects,
               COUNT(DISTINCT CASE WHEN p.qualification_status='Qualified' THEN p.name END) qualified_prospects,
               COUNT(DISTINCT CASE WHEN p.qualification_status='Manually Approved' THEN p.name END) manually_approved_prospects,
        ...
        ORDER BY total_prospects DESC
    """)
    return columns, data, None, _bar_chart(data, "research_arena", "total_prospects", "Prospects")
...
def _execute_crm_deal_conversion_detail(filters):
    ...
    data = _sql(f"""
        SELECT p.name sei_prospect, {_prospect_arenas_expr("p")} research_arena, {_prospect_playbooks_expr("p")} playbooks, (SELECT s.name FROM {utils.table('SEI Signal')} s WHERE s.prospect = p.name ORDER BY (SELECT MAX(f.source_date) FROM {utils.table('SEI Signal Observed Fact')} f WHERE f.parent=s.name) DESC, s.creation DESC LIMIT 1) primary_signal,
...
def _execute_crm_context_missing(filters):
    ...
    rows = []
    rows.extend(_missing_context_rows_for_doctype("CRM Lead", ["sei_prospect", "sei_research_arena", "sei_playbook", "sei_qualification_summary"], "CRM Lead"))
    rows.extend(_missing_context_rows_for_doctype("CRM Deal", ["sei_prospect", "sei_research_arena", "sei_playbook", "sei_primary_signal"], "CRM Deal"))
    if utils.has_doctype("SEI Prospect"):
        rows.extend(_sql(f"""
            SELECT 'SEI Prospect' record_type, name record,
                   CONCAT_WS(', ', CASE WHEN {_prospect_has_crm_leads_expr('p')} AND COALESCE({_prospect_arenas_expr('p')}, '') = '' THEN 'research_arena' END) missing_context,
                   modified
            FROM {utils.table('SEI Prospect')} p
...
REPORT_EXECUTORS = {
    ...
    "Prospects by Research Arena": _execute_prospects_by_research_arena,
